...
class PricingAPITests(APITestCase, TestUtils):
    ...
    def test_get_current_pricing(self):
        ...
        self.assertEqual(response.data['fees'][str(fee_beam_eur.id)], {'EUR': 0.3})
        self.assertEqual(response.data['fees'][str(fee_beam_gbp.id)], {'GBP': 0.2})
        ...
        self.assertEqual(response.data['exchange_rate_id'], exchange_rate.id)
        self.assertEqual(response.data['fees'][str(fee_bae_usd.id)], {'USD': 0})
